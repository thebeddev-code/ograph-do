...
export const createTodo = (todoPayload: CreateTodoPayload) => {
  return api.post(`/todos`, todoPayload);
};
...
export const useCreateTodo = ({ mutationConfig }: UseCreateTodoOptions) => {
        ...
        context.client.setQueryData(queryKey, (cache) =>
          cache
            ? {
                data: [
                  ...previousTodos,
                  { id: previousTodos.at(-1).id + 1, ...todoPayload },
                ],
                meta: cache.meta,
              }
            : cache,
